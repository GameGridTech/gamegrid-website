...
// Curved Icon Marquee Card Component
    ...
    <Card className="bg-black text-white border-white/10 rounded-3xl">
      ...
      <CardContent className="flex items-center justify-center pb-8">
        <div className="relative rounded-2xl overflow-hidden mt-10 w-full">
          ...
          {/* Overlay CTA: button + email input */}
...
// Billing toggle component (Monthly / Yearly) - kept for future use
...
// Pricing Card types and component - kept for future use
...
function SportsImageSlider() {
  ...
  // GSAP ticker for continuous motion
  useEffect(() => {
...
// Placeholder Cards Slider Component
function PlaceholderCardsSlider() {
  ...
  const placeholderCards = [
    { id: 1, metric: "+100,000", title: "Stats Expected in Year One", description: "Tracks stats across local leagues, tournaments, and rec teams in real time." },
    { id: 2, metric: "10,000+", title: "Players Projected", description: "Forecasted to serve over 10K players, giving each one visibility, rankings, and season stats." },
    { id: 3, metric: "70%", title: "Faster League Management", description: "Beta testers report setting teams, schedules, and rosters 70% faster than traditional methods like spreadsheets and group chats." },
    { id: 4, metric: "500+", title: "Leagues Targeted", description: "Working with community leaders and organizers to bring GameGrid to 500+ active leagues in basketball, soccer, and volleyball." },
    { id: 5, metric: "40%", title: "Higher Player Retention (Pilot Data)", description: "Leagues using our beta tools saw improved attendance and return rate—players stay engaged when their performance is tracked." },
    { id: 6, metric: "94%", title: "Beta User Satisfaction", description: "In early trials, 94% of admins and players said they'd use GameGrid again, citing speed, simplicity, and stat accuracy." },
    { id: 7, metric: "24/7", title: "Real‑Time Availability", description: "Always-on stat tracking and standings so players and admins can check updates anytime." },
    { id: 8, metric: "3+", title: "Sports at Launch", description: "Focused rollout across basketball, soccer, and volleyball with more sports planned." },
  ];
  ...
  useEffect(() => {
    ...
    const track = containerRef.current;
    if (!track) return;
    xRef.current = -cycleWidthRef.current / 2;
    gsap.set(track, { x: xRef.current });
    ...
    const update = () => {
      ...
      const dx = (speedPxPerSec / 60) * delta;
      xRef.current += dx;
      if (xRef.current <= -cycle) xRef.current += cycle;
      gsap.set(track, { x: xRef.current });
    };
    gsap.ticker.add(update);
    return () => { gsap.ticker.remove(update); };
